...
// Create NATS client.
const nc = ncFactory(clusterID, clientID, subject, updatedDeploymentVariables);
...
export async function updatedDeploymentVariables(msg) {
  try {
    ...
    const values = mapCustomEnvironmentVariables(
      deployment,
    ...
    const buf = Buffer.from(JSON.stringify(environmentVariables));
